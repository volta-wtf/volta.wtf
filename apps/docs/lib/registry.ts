...
import { registryItemFileSchema, registryItemSchema } from "@/registry/config/schema"
...
function resolveRegistryPath(filePath: string): string {
  ...
  // If the path starts with "registry/", it's relative to monorepo root
  // apps/docs is 2 levels deep, so go up 2 levels
...
export async function getRegistryItem(name: string) {
  ...
  for (const file of item.files) {
    ...
    // Resolve the full path and then make it relative to current working directory
    const fullPath = resolveRegistryPath(file.path)
...
async function getFileContent(file: z.infer<typeof registryItemFileSchema>) {
  // Resolve the path from monorepo root
  const fullPath = resolveRegistryPath(file.path)
